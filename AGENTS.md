...
- `src/utils/` — host-neutral logic shared by both entries: `models.ts` (`fetchModels` + fallbacks, `toModel`/`toConfigModel`), `credentials.ts` (AK/SK resolution, `/connect` storage parsing, `resolveBase`), `constants.ts` (ids), `paths.ts`, plus the API modules (`signer.ts`, `discover.ts`, `cache.ts`, `vision.ts`, `i18n.ts`).
...
- `src/utils/discover.ts` — model discovery via agent-center chain (`useragents` → `detail`), requires `Agent-Type: AgentCenter` header. Returns `[]` when the account has no enabled models (no hardcoded fallback).
